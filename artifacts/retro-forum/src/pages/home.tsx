          ...
          <tbody>
            {cat.forums.map((forum, idx) => (
              <tr key={forum.id} className={idx % 2 === 0 ? "blox-row-odd" : "blox-row-even"} data-testid={`forum-row-${forum.id}`}>
                ...
                <td style={{ textAlign: "center" }} className="blox-meta">{forum.threads.toLocaleString()}</td>
                <td style={{ textAlign: "center" }} className="blox-meta">{forum.posts.toLocaleString()}</td>
                <td className="blox-meta" style={{ whiteSpace: "nowrap", fontSize: "11px" }}>
                  {forum.lastPost ? (
                    <>
                      ...
                      <br />
                      by{" "}
                      <Link href={`/thread/${forum.lastPost.threadId}`} data-testid={`last-post-link-${forum.id}`}>
                        <b>{forum.lastPost.author}</b>
                      </Link>
                      <br />
                      ...
                      {" "}
                      <Link href={`/thread/${forum.lastPost.threadId}`} style={{ fontSize: "10px" }}>
                        View last post
                      </Link>
                    </>
                  ) : (
